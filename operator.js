...
async function runPrivateBrowsingSearch(){
 document.getElementById('q').value='Private Browsing'; logEl.textContent=''; renderRows([]); setProgress(0);
 document.getElementById('status').textContent='Searching all drives for Private Browsing records'; broadcast('cue','reset'); broadcast('cue','searchPrivateBrowsing');
 const steps=['Preparing artefact search: Private Browsing','Mounting SUSPECT-PC.E01 read-only','Scanning NTFS master file table','Scanning Documents and Settings','Scanning Application Data and browser cache folders','Scanning Recycle Bin and Temporary Internet Files','Scanning unallocated clusters','Private Browsing records located','Sending match list to analysis workspace'];
 ...
 document.getElementById('status').textContent='Search complete: Private Browsing records located'; selectFile(0);
}
...
function runSearchFromBox(){ document.getElementById('q').value.toLowerCase().includes('decomposition')?runDecompSearch():runPrivateBrowsingSearch(); }
...
document.addEventListener('keydown',e=>{ if(e.key==='Enter')runSearchFromBox(); if(e.key===' ')openFile(0); if(e.key==='F1'){e.preventDefault();runPrivateBrowsingSearch()} if(e.key==='F2'){e.preventDefault();broadcast('cue','privateBrowsingHit')} if(e.key==='F3'){e.preventDefault();broadcast('cue','inspectArchive')} if(e.key==='F4'){e.preventDefault();runDecompSearch()} if(e.key==='F5'){e.preventDefault();broadcast('cue','private')} if(e.key==='F6'){e.preventDefault();broadcast('cue','note')} if(e.ctrlKey&&e.key.toLowerCase()==='d'){e.preventDefault();openDirector()} if(e.key.toLowerCase()==='f')toggleFull(); });
