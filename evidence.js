...
function hex(){setStatus('File signature viewer analysing recovered bytes'); win('hex','File Signature Viewer',null,null,520,320,`<div class="moduleTitle"><span class="moduleIcon">0x</span>Signature Analysis</div><div class="hexview">00000000  50 52 49 56 41 54 45 00  53 51 4C 69 74 65 20 66  PRIVATE.SQLite f\n00000010  08 00 4A 99 6E 3A 42 91  A1 22 08 00 00 00 12 00  ..J.n:B..\"......\n00000020  00 00 62 61 63 6B 75 70  2E 64 61 74 00 00 00 00  ..backup.dat....\n00000030  53 51 4C 69 74 65 20 66  6F 72 6D 61 74 20 33 00  SQLite format 3.</div><p><b>Result:</b> compressed container / embedded SQLite records suspected.</p>`,{status:'Signature match'}); appendLog('Embedded database marker found');}
function privateBrowsingHit(){flash(); notify('Private Browsing Artefacts Found','Recovered browser data contains <b>Private Browsing</b> records.'); setStatus('Private Browsing artefacts located'); win('hit','Keyword Match Viewer',null,null,720,430,`<div class="moduleTitle"><span class="moduleIcon">✓</span>Private Browsing Artefacts</div><table><tr><th>Filename</th><th>Recovered path</th><th>Status</th></tr><tr><td>Private Browsing</td><td>C:\\Documents and Settings\\Patrick\\Local Settings\\Temp</td><td>Partial access</td></tr><tr><td>PrivateBrowsing.db</td><td>Application Data\\Browser Cache\\Backups</td><td>Archive header recovered</td></tr><tr><td>PrivateBrowsing_Recovered.dat</td><td>Temporary Internet Files</td><td>Locked</td></tr><tr><td>private_browsing_notes.txt</td><td>My Documents\\Notes</td><td>Accessible</td></tr></table><p><b>Important:</b> records recovered from the suspect computer image. No external device is implied.</p>`,{status:'Matches located'}); appendLog('Private Browsing artefacts located');}
...
function hardReset(){ startup(); }
function onCue(c){
  const map={reset:hardReset,searchPrivateBrowsing,metadata,archive,hex,privateBrowsingHit,inspectArchive,decompStart,keywordViewer,sqlite,private:privateView,recoveryLog,note};
  if(map[c]) map[c]();
}
...
document.querySelector('.wallTitle')?.addEventListener('dblclick',()=>hardReset());
document.getElementById('statusLeft')?.addEventListener('dblclick',()=>hardReset());
document.addEventListener('keydown',e=>{ if(e.ctrlKey && e.shiftKey && e.key.toLowerCase()==='r'){ e.preventDefault(); hardReset(); }});
startup(); splash();
...
function resetWorkspace() {
    ...
    statusLeft.textContent =
        "Connected to operator workstation";

    setStatus("Analysis queue idle");
